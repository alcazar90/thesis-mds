...
\ca{\textbf{Terminar introducción de interfaz MDP y notación}. Basarse en una presentación como las secciones 2.1 y 2.2 de tesis de Schulman \cite{schulman2016optimizing}}
...
\ca{\textbf{Destacar el objetivo a resolver por RL desde un comienzo} -> What's the objective? maximize the expected reward...}
...
\ca{\textbf{Agregar introducción antes de 3.2.1.}. Básicamente el objetivo de resolver directamente el problema anterior optimizando la policy.}

\ca{En paper policy gradient methods with function approximation se resume de buena manera:}. ``the policy might be represented by a neural network whose input is a representatioon of the state, whose output is action selectioon probabilities, and whose weights are the policy parameters.'' \cite{sutton1999policy} Reusar una descripcción de este estilo sobre la policy representada por una función de aproximación parametrizada. \\

% Tesis de Schulman final capítulo 4 GAE
\noindent Destacar esto...``Policy gradient methods provide a way to reduce reinforcement learning to stochastic gradient descent, by providing unbiased gradient estimation...success...has been limited, largely due to their high sample complexity...''\ca{Vincular con Figure~\ref{fig:rl-model-free-taxonomy} sobre la diferencia con otros métodos.} \\
...
\noindent \ca{\textbf{Explicar porqué reward-to-go reduce la varianza.}} A naive estimate of $R(\tau)$ uses the total trajectory reward, which means that every action is weighted by the total reward. Even if the gradient estimator is unbiased, we need to consider the variance of the estimator.
...
\noindent \textbf{What baseline to use?} Estimate the \textit{advantage function} $A_{t} = \sum_{t=0}^{T-1} r_{t}^{'}-b(s_{t})$...\ca{\textbf{TODO:} introducir términos para (i) $R(\tau)$ y (ii) $b(s_{t})$.}.
...
than the expected return under the current policy...\ca{parafrasear, calcado de L3}.
...
\ca{\textbf{Introducir actor-critic como método de aprender en conjunto la política y la función de valor.} Mostrar variante de REINFORCE aproximando
la función de valor y que se puede aprender con MSE. Acá retomar Figure~\ref{fig:rl-model-free-taxonomy} que es donde se junta policy optimizatioon con dynamic programing.} \\
...
\ca{\textbf{Sección Incompleta:} No me convence, si va la completo al final, solo demostrar como el área de RL ha realizado modificaciones para adaptar redes neuronales lo más parecido a lo que es en aprendizaje supervisad.}
...
% \begin{sourcecode}[\label{agent-interface}]{python}{Actor-Critic Interface.}
...
% \begin{sourcecode}[\label{training-loop}]{python}{Training-Loop}
%     x = 2
% \end{sourcecode}
...
\ca{\textbf{Sección Incompleta:} Introducir trust region para resolver problema anterior y solución práctica que deriva en PPO.}

In 2015, Schulman \cite{schulman2015trust} introduced Trust Region Policy Optimization (TRPO) to address the challenge of balancing policy improvement with stability in reinforcement learning \ca{Cambiar este párrafo...}. 
...
\ca{\textbf{Sección Incompleta:} acá er muy breve y explicar como el humano puede interactuar dentro de la interfaz MDP, agregar diagrama con la interacción vía modelo de reward y el ciclo presentado en Figure~\ref{fig:anatomy-rl-algo}}.
...
\ca{Resumir muy breve todo lo anterior.}
